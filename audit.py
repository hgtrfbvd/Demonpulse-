...
SEVERITY = {
    "LOGIN":"INFO","LOGOUT":"INFO","LOGIN_FAIL":"WARN","TOKEN_EXPIRE":"INFO",
    "PASSWORD_CHANGE":"WARN","BET_PLACED":"INFO","BET_SETTLED":"INFO",
    "BET_CANCELLED":"WARN","BANKROLL_RESET":"CRITICAL","BANKROLL_SET":"WARN",
    "SETTINGS_UPDATE":"WARN","USER_CREATE":"WARN","USER_DELETE":"CRITICAL",
    "RACE_LOCK":"INFO","RACE_SCRATCH":"WARN","CACHE_CLEAR":"INFO",
    "SWEEP_MANUAL":"INFO","LEARNING_PROMOTE":"WARN","ACCESS_DENIED":"WARN",
    "ERROR":"ERROR","ENV_MODE_SWITCH":"CRITICAL","TEST_PURGE":"WARN",
...
def log_event(user_id, username, event_type, resource="", data=None, ip=None, severity=None) -> dict:
    entry = {
        ...
        "resource":   resource,
        "data":       {**(data or {}), "env_mode": env.mode},
        "ip":         ip,
    ...
    if sev == "CRITICAL": log.critical(msg)
    elif sev == "ERROR":  log.error(msg)
    elif sev == "WARN":   log.warning(msg)
    else:                 log.info(msg)

    # audit_log is in _ALWAYS_LIVE_TABLES — db.T() will NOT prefix it
    try:
        import json
        from db import get_db, safe_query, T
        safe_query(lambda: get_db().table(T("audit_log")).insert({
...
def get_audit_summary() -> dict:
    try:
        from db import get_db, safe_query, T
        recent = safe_query(
            lambda: get_db().table(T("audit_log")).select("event_type,severity,created_at")
        ...
        for row in recent:
            ...
            s = row.get("severity","INFO")
            by_sev[s] = by_sev.get(s,0) + 1
        return {"total":len(recent),"by_type":by_type,"by_severity":by_sev,"critical_count":by_sev.get("CRITICAL",0)}
    except Exception:
        return {"total":len(_BUFFER),"by_type":{},"by_severity":{},"critical_count":0}
