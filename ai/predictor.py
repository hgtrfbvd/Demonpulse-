...
Feature-engine model (model_version = "v2_feature_engine"):
  - Weighted multi-signal score combining:
      ...
      race_shape_fit          × 0.12
      enrichment class fit    × 0.08  (FormFav; flagged non-authoritative)
      enrichment form trend   × 0.08  (FormFav; flagged non-authoritative)
    - (collision_risk_score)  × 0.10  (subtracted; greyhounds only)
...
# Weights for the v2_feature_engine model
_V2_WEIGHTS = {
    "implied_probability":          0.30,
    "early_speed_score":            0.12,
    "late_speed_score":             0.12,
    ...
    "race_shape_fit":               0.12,
    "enrichment_race_class_fit":    0.08,   # FormFav — non-authoritative; clearly flagged
    "enrichment_win_pct_norm":      0.08,   # FormFav — normalised win pct
    # collision risk subtracted (greyhound only)
...
def predict_from_snapshot_v2(
    ...
    Args:
        ...
        collision_metrics: optional per-runner collision metric dicts (greyhounds only)
...
def _v2_feature_score(features: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    Weights (see _V2_WEIGHTS at module top):
        ...
        enrichment_race_class_fit    × 0.08   (FormFav — non-authoritative)
        enrichment_win_pct_norm      × 0.08   (FormFav — non-authoritative, normalised)
      - collision_risk_score         × 0.10   (subtracted; greyhounds only)
    ...
    win_pcts = [
        float(f.get("enrichment_win_pct") or 0.0) for f in features
    ]
    ...
    for feat in features:
        ...
        class_fit     = float(feat.get("enrichment_race_class_fit") or 0.0)
        win_pct_norm  = (
